...
string response_from_tracker_generator(string tracker_ip, vector<pair<string, int> > my_available_peers) {
  ...
  for (unsigned int i = 0; i < my_available_peers; i++) {
    ...
    if (message[0] != 'X') {
      ...
      if (my_info.read_data(message) == 1) {
        for (unsigned int i = 0; i < loged_in_database_ph.size(); i++) {
          ...
          if (my_custom_send(my_info.data[0], my_info.data[1], response_from_tracker_generator(tracker_ip, response_peers)) == 0) {
            cout << "error al enviar la respuesta" << endl;
          }
          else {
            cout << "exito al enviar la respuesta : " << response_from_tracker_generator(tracker_ip, response_from_tracker_generator) << endl;
          }
        }
      }
    }
